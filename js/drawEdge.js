...
function drawEdgeFermion(ctx, e) {
  ...
  if (e.type == "Fermion")
    ...
    drawArrowHead(ctx, e.midpoint(), eVec.angle()+Math.PI, arrowSize);
}
...
function drawEdgeGluon(ctx, e) {
  drawEdgeGenericCurve(ctx, e,
    function (t) {
      const nLoops = 6;
      const pi = Math.PI;
      const amp = 0.05, k = pi*(2*nLoops + 1);
...
function drawMomentumArrow(ctx, e) {
  ...
  let v1vec = toVec(v1);
  const f1 = (1-e.arrowFrac)/2, f2 = 1-f1;
...
function drawCurvedMomentumArrow(ctx, e) {
  ...
  const f1 = e.arrowFrac, f2 = (1-f1)/2;
  ...
  const arrowSize = 15;
  if (e.reverseMomentum)
    drawArrowHead(ctx, p2, ang2-Math.PI/2, arrowSize);
  else
    drawArrowHead(ctx, p1, ang1+Math.PI/2, arrowSize);
}
